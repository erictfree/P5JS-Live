// The original ASCII + Plasma example, retained for library installs and saved-project upgrades.

export const ASCII_PLASMA_SOURCE = `// %% patch asciiNoise
...
export function upgradeLegacyPlasma(source) {
  ...
  const newCell = cellRange(ASCII_PLASMA_SOURCE);
  if (!oldCell || !newCell) return source;

  const replacement = ASCII_PLASMA_SOURCE.slice(newCell.start, newCell.end).trimEnd();
